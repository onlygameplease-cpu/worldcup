...
if os.path.exists("data1"):
    for f in os.listdir("data1"):
        if f.endswith(".csv"):
            league_name = os.path.splitext(f)[0]
            DATA_FILES[league_name] = f"data1/{f}"
DATA_FILES["ALL (gộp tất cả)"] = None

ALL5_LEAGUES = [k for k, v in DATA_FILES.items() if v is not None]
            ...
            for mkt in book_payload:
                ...
                for odd_set in (mkt.get("odds") or []):
                    if not isinstance(odd_set, dict):
                        continue
                    point = odd_set.get("handicap") or odd_set.get("total") or odd_set.get("line") or odd_set.get("spread")
                    if "home" in odd_set and "away" in odd_set:
                        ...
                        for k, v in odd_set.items():
                            if k not in ["handicap", "total", "line", "spread"]:
...
def load_df(path: str, mtime: float) -> pd.DataFrame:
    ...
    # Map alternative column names often found in Extra Leagues CSVs
    rename_map = {
        "Home": "HomeTeam",
        "Away": "AwayTeam",
        "HG": "FTHG",
...
tabs = st.tabs(["Goals", "H2H", "Summary", "Odds API.io", "Handicap (AH)"])
...
with tabs[2]:
    ...
    with right:
        ...
        default_prompt = (
            ...
            f"- (Optional) Recent stats: \n"
            f"\n"
            f"RULES:\n"
            f"- Use ONLY the pasted INPUT LINES and the RAW MARKET SUMMARY above. No external knowledge.\n"
            f"- You may output picks ONLY for markets that are explicitly present in INPUT LINES or clearly present in RAW MARKET SUMMARY.\n"
            f"- Market lean rule (if prices are included like Over 0.93 / Under 1.05): the LOWER price indicates the market-lean side.\n"
            f"- Use statistical hit rates from the provided tables to guide your picks.\n"
            f"- If Odds API snapshot is provided and matched=true, read only that matched event's odds/lines directly and prioritize them over guesses.\n"
            f"- If the matched odds snapshot comes from odds-api.io and INPUT LINES are still blank, treat RAW MARKET SUMMARY as valid input if it contains usable bookmaker market/line/price pairs.\n"
            f"- If RAW MARKET SUMMARY shows usable bookmaker market/line/price pairs, do NOT say odds are missing just because the INPUT LINES section is blank.\n"
            f"- If Odds API snapshot says matched=false, do not claim to use fetched market odds for this match.\n"
            f"- ODDS FILTER: Only recommend a pick if the chosen side price (HK) >= 0.75. If the chosen side price < 0.75 or missing -> NO BET (or choose another market that meets >=0.75).\n"
            f"- Only say odds are 'missing' if the matched odds snapshot truly has no usable bookmaker markets, lines, or prices after checking the raw matched event object.\n"
            f"{provider_extra_rules}"
            f"- NOTE must explain the MAIN pick only. If you mention the backup, prefix with 'Backup:' clearly.\n"
            f"- If you mention market lean, it must refer to the SAME market and the SAME side as the pick.\n"
            f"- If your pick goes AGAINST market lean, you MUST explicitly say 'contrarian vs market lean <side>' (or output NO BET).\n"
            f"- Use market lean as a tie-breaker or confidence booster; do not rely on it alone if stats contradict.\n"
            f"- If information is unclear/contradictory -> output NO BET.\n"
            f"\n"
            f"OUTPUT (max 2 lines; picks only, no price):\n"
            f"MAIN: <one pick OR NO BET>\n"
            f"BACKUP: <one pick OR '-'> | NOTE: <1 short sentence about MAIN only; optional 'Backup:' second clause>\n"
            f"\n"
            f"Pick format (choose ONE):\n"
            f"- Goals: Over <line> OR Under <line>\n"
            f"- 1H: Over <line> OR Under <line>\n"
            f"- Corners: Over <line> OR Under <line>\n"
        ...
        if run_clicked or copy_clicked:
            ...
            if copy_clicked:
                import json
                st.warning("Vì bạn đang chạy app qua mạng nội bộ (IP) nên nút Copy góc phải có thể bị trình duyệt chặn. Hãy click chuột vào khung dưới, bấm **Ctrl + A** để bôi đen tất cả, rồi **Ctrl + C** để copy nhé:")
                st.text_area("Nội dung Payload (JSON)", value=json.dumps(payload, ensure_ascii=False, separators=(',', ':'), default=str), height=150, key="payload_text_area")
