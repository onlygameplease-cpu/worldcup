...
from prompt_builder import build_prompt_odds_block
...
DATA_FILES = {
    ...
    "World Cup": "dataworldcup/advanced_stats.csv",
...
st.set_page_config(page_title="World Cup Advanced Stats", layout="wide")

TASTE_SKILL_CSS = """

<style>

/* TASTE-SKILL AESTHETICS */
...
st.title("🏆 World Cup Advanced Stats Dashboard")
...
def _event_teams_from_odds_item(item: dict) -> tuple[str, str]:
    ...
    return (
        ...
        normalize_team_name(item.get("home_team", "")),

        normalize_team_name(item.get("away_team", "")),
...
def find_matching_odds_api_io_event(events: list | None, home_team: str, away_team: str) -> dict | None:
    ...
    for item in events:
        ...
        elif normalize_team_name(home_name) in normalize_team_name(home_team) or normalize_team_name(home_team) in normalize_team_name(home_name):
        ...
        elif normalize_team_name(away_name) in normalize_team_name(away_team) or normalize_team_name(away_team) in normalize_team_name(away_name):
...
def extract_input_lines_from_matched_odds(matched_odds: dict | None) -> dict:
    ...
    if totals_best:
        ...
        if point is not None and (over_price is not None or under_price is not None):
            out["goals"] = (
                f"BINARY QUESTION FOR TOTALS:\n"
                f"Dựa vào dữ liệu nâng cao, hãy chọn 1 trong 2 quyết định sau cho kèo Tài Xỉu:\n"
                f"1. Chọn Over {_format_num(point)} (@{_format_num(over_price)})\n"
                f"2. Chọn Under {_format_num(point)} (@{_format_num(under_price)})\n"
                f"Chỉ trả về 1 lựa chọn duy nhất có Expected Value cao hơn và giải thích vì sao."
            )
    ...
    if spreads_best:
        ...
        if line is not None and (home_price is not None or away_price is not None):
            away_line = -line if line != 0 else 0
            out["handicap"] = (
                f"BINARY QUESTION FOR HANDICAP:\n"
                f"Dựa vào dữ liệu nâng cao, hãy chọn 1 trong 2 quyết định sau cho kèo Handicap:\n"
                f"1. Chọn {home} {_format_num(line)} (@{_format_num(home_price)})\n"
                f"2. Chọn {away} {_format_num(away_line)} (@{_format_num(away_price)})\n"
                f"Chỉ trả về 1 lựa chọn duy nhất có Expected Value cao hơn và giải thích vì sao."
            )
...
def ah_points(goals_for: float, goals_against: float, line: float) -> float:
    ...
    Tráº£ vá» Ä‘iá»ƒm cÆ°á»£c Asian Handicap cho 'Ä‘á»™i mÃ¬nh' theo line:
    ...
    # line .0 hoáº·c .5 => 1 bet

    if q % 2 == 0:

        return _single(q / 4)

    # line .25 hoáº·c .75 => split 2 bet
...
# Force reload after updating Haiti FIFA rank
import copy

def filter_matched_odds_with_poisson(matched_odds, team_a_stats, team_b_stats):
    if not isinstance(matched_odds, dict) or not matched_odds.get("matched"):
        return matched_odds
        
    # Deepcopy to prevent mutating the original st.session_state payload!
    matched_odds = copy.deepcopy(matched_odds)
    ...
    team_a = event.get("home_team", "Team A")
    team_b = event.get("away_team", "Team B")
        
    home_xg = float(team_a_stats.get("xG") or team_a_stats.get("avg_xG") or 1.5)
    away_xg = float(team_b_stats.get("xG") or team_b_stats.get("avg_xG") or 1.2)
    ev_funcs = predict_poisson_market_probs(home_xg, away_xg, home_stats=team_a_stats, away_stats=team_b_stats)
    
    # --- CIRCUIT BREAKER: HIGH BLOWOUT RISK FILTER ---
    is_blowout = check_high_blowout_risk(team_a_stats, team_b_stats)
    
    t_out, s_out, btts_out, picks = filter_main_odds_lines(bookmakers, ev_funcs, is_blowout_risk=is_blowout)
    
    if is_blowout:
        picks["is_blowout_risk"] = True
        picks["debug"].append("⚠️ CIRCUIT BREAKER: HIGH BLOWOUT RISK detected. Poisson picks are preserved but use caution.")
        
        rank_a = float(team_a_stats.get("latest_Rank") or 100)
        rank_b = float(team_b_stats.get("latest_Rank") or 100)
        xg_a = float(team_a_stats.get("avg_xG") or 1.0)
        xg_b = float(team_b_stats.get("avg_xG") or 1.0)
        fav_side = "home" if (rank_a < rank_b if rank_a != 100 and rank_b != 100 else xg_a > xg_b) else "away"
    ...
    # --- DEBUG INFO ---
    picks["_debug_home_xg"] = round(home_xg, 3)
    picks["_debug_away_xg"] = round(away_xg, 3)

    # --- COMBO RECOMMENDER ---
    def recommend_combo(pks, blowout_risk):
        tp = pks.get("totals_pick") or ""
        sp = pks.get("spreads_pick") or ""
        bp = pks.get("btts_pick") or ""
        
        if blowout_risk:
            fav = pks.get("blowout_fav_side", "?")
            return (f"⚠️ CẢNH BÁO VỠ TRẬN: Chênh lệch đẳng cấp rất lớn.\n"
                    f"📊 Poisson chọn: {tp or 'N/A'} | {sp or 'N/A'}\n"
                    f"🛡️ Kèo an toàn: {fav} Handicap & BTTS No\n"
                    f"👉 Nếu Poisson và Kèo an toàn ĐỒNG THUẬN → Tự tin đánh. Nếu MÂU THUẪN → Cân nhắc skip hoặc giảm stake.")
            
        if "Over" in tp and "Yes" in bp:
            return f"🔥 {tp.split('(@')[0].strip()} & BTTS Yes\n👉 Lý do: Trận đấu đôi công, nhiều bàn thắng, tương quan Tài cực mạnh."
            
        if "Under" in tp and "No" in bp:
            return f"🛡️ {tp.split('(@')[0].strip()} & BTTS No & (Tùy chọn: {sp.split('(@')[0].strip()})\n👉 Lý do: Thế trận bế tắc, chặt chẽ, dễ hòa hoặc thắng cách biệt nhỏ."
            
        return "⚖️ Không có kèo tương quan mạnh. Lựa chọn đánh đơn lẻ."
        
    picks["recommended_combo"] = recommend_combo(picks, is_blowout)
    
    # Rebuild bookmakers to only include the best lines
    new_bookmakers = []
    if t_out or s_out or btts_out:
        markets = []
        if t_out and picks.get("totals_pick"):
            markets.append({"key": "totals", "outcomes": t_out})
        if s_out and picks.get("spreads_pick"):
            markets.append({"key": "spreads", "outcomes": s_out})
        if btts_out and picks.get("btts_pick"):
            markets.append({"key": "btts", "outcomes": btts_out})
        if markets:
            new_bookmakers.append({"key": "poisson_filtered_bookmaker", "title": "Poisson Filtered", "markets": markets})
        
    matched_odds["bookmakers"] = new_bookmakers
    matched_odds["poisson_picks"] = picks
    if matched_odds.get("event"):
        matched_odds["event"]["bookmakers"] = new_bookmakers
...
def team_ah_last(df: pd.DataFrame, team: str, n: int, side_filter: str | None = None) -> pd.DataFrame:
    ...
    def _row(r: pd.Series) -> pd.Series:
        ...
        if r.get("HomeTeam") == team:
            ...
            gf = r.get("HomeScore", float("nan"))

            ga = r.get("AwayScore", float("nan"))
            ...
            gf = r.get("AwayScore", float("nan"))

            ga = r.get("HomeScore", float("nan"))
...
def run_ou_backtest(
    ...
    if "Date" in work.columns:
        ...
        work = work.sort_values("Timestamp", ascending=True).reset_index(drop=True)
...
def call_ai_api(api_url: str, api_key: str, payload: dict):
    ...
    if is_gemini:
        ...
        adv_metrics = (payload or {}).get("advanced_metrics", None)
        ...
        if adv_metrics is not None:
            ...
            try:
                ...
                adv_metrics_str = json.dumps(adv_metrics, ensure_ascii=False, indent=2)

            except Exception:

                adv_metrics_str = str(adv_metrics)

            parts.append("Advanced Metrics (JSON):\n" + adv_metrics_str)
    ...
    attempts = 9999 if is_gemini else 1
    ...
    for _ in range(attempts):
        ...
        try:
            ...
            r = requests.post(url, headers=headers, json=req_json, timeout=180)
            ...
            if r.status_code in (429, 500, 502, 503, 504):
                import time
                if _ < attempts - 1:
                    wait_time = min(2 ** _ + 2, 10)  # Capped at 10 seconds max
                    if r.status_code == 503:
                        wait_time = 5  # Quick retry for 503
                    try:
                        import streamlit as st
                        st.toast(f"AI Server Overloaded ({r.status_code}). Retrying in {wait_time}s... (Try {_ + 1})", icon="⏳")
                    except: pass
...
def call_openai_api(model: str, api_key: str, payload: dict):
    ...
    try:
        ...
        client = OpenAI(api_key=api_key, base_url=api_url if api_url else None)
...
def infer_market_family(bet_text: str) -> str:
    ...
    if "tháº»" in s or "card" in s:

        return "cards"

    if "gÃ³c" in s or "corner" in s:
    ...
    if any(tok in s for tok in ["tÃ i", "xá»‰u", "trÃªn", "dÆ°á»›i", "over", "under", "o ", "u "]):

        return "goals_ou"

    if any(tok in s for tok in ["+", "-", " pk", " 0", "Ä‘á»“ng banh"]):
...
def load_record_history(_: float = 0.0) -> dict:
    ...
    for sheet, df in sheet_frames:
        ...
        if sheet.strip().lower() in {"form responses 1", "tÃ­nh xiÃªn"}:
...
def render_ai_result(result):
    ...
    st.markdown("### ðŸŽ¯ PRIMARY RECOMMENDATION")
    ...
    if alternatives:
        ...
        st.markdown("### ðŸ” ALTERNATIVE BETS")
...
def load_df(path: str, mtime: float) -> pd.DataFrame:
    ...
    if {"HomeScore", "AwayScore"}.issubset(df.columns):

        df["TG"] = _num(df["HomeScore"]) + _num(df["AwayScore"])
    ...
    if {"HomeScore", "AwayScore"}.issubset(df.columns):
        ...
        df["BTTS"] = (_num(df["HomeScore"]) > 0) & (_num(df["AwayScore"]) > 0)

    

    # --- Asian Handicap results (dá»±a trÃªn AHh) ---

    if "AHh" in df.columns and {"HomeScore","AwayScore"}.issubset(df.columns):

        df["AHh"] = pd.to_numeric(df["AHh"], errors="coerce")

        # KÃ¨o cho HOME (dÃ¹ng AHh trá»±c tiáº¿p)

        df["AH_home_pts"] = df.apply(

            lambda r: ah_points(r["HomeScore"], r["AwayScore"], r["AHh"]), axis=1

        )

        # KÃ¨o cho AWAY (line ngÆ°á»£c láº¡i)

        df["AH_away_pts"] = df.apply(

            lambda r: ah_points(r["AwayScore"], r["HomeScore"], -r["AHh"]), axis=1
    ...
    # --- Corner Asian Handicap results (dá»±a trÃªn AHCh, corners) ---

    # AHCh: line handicap theo sá»‘ gÃ³c cho Home (Ã¢m = Home cháº¥p gÃ³c).

    # Dá»±a trÃªn HC/AC (Home/Away corners).

    if "AHCh" in df.columns and {"HC","AC"}.issubset(df.columns):

        df["AHCh"] = pd.to_numeric(df["AHCh"], errors="coerce")

        # KÃ¨o cho HOME (dÃ¹ng AHCh trá»±c tiáº¿p)
...
# chá»‰ gá»™p 5 giáº£i nÃ y thÃ´i

ALL5_LEAGUES = ["EPL", "LaLiga", "Serie A", "Bundesliga", "Ligue 1", "Greece", "Netherlands", "Portugal", "Scotland", "Turkey", "Belgium"]

if league == "ALL (gá»™p 5 giáº£i)":
...
if missing:
    ...
    st.error("KhÃ´ng tháº¥y file:\n" + "\n".join(missing))

    st.stop()

# load (cache theo tá»«ng file)
...
for p in paths:
    ...
    dfi["League"] = league if league != "ALL (gá»™p 5 giáº£i)" else os.path.splitext(os.path.basename(p))[0]
...
# Danh sách 48 đội (dự kiến/đã chốt) tham dự World Cup 2026 để làm mượt giao diện

WC_2026_TEAMS = [

    "Czech Republic", "Mexico", "South Africa", "South Korea", "Switzerland", "Bosnia-Herzegovina",

    "Canada", "Qatar", "Scotland", "Brazil", "Haiti", "Morocco",

    "Turkey", "Paraguay", "United States", "Australia", "Germany", "Ecuador",

    "Ivory Coast", "Curacao", "Sweden", "Netherlands", "Tunisia", "Japan",

    "Belgium", "Egypt", "Iran", "New Zealand", "Spain", "Uruguay",

    "Cape Verde", "Saudi Arabia", "France", "Norway", "Senegal", "Iraq",

    "Austria", "Argentina", "Algeria", "Jordan", "Portugal", "Colombia",

    "Congo DR", "Uzbekistan", "Croatia", "England", "Ghana", "Panama"

]

teams = sorted(pd.unique(pd.concat([df["HomeTeam"], df["AwayTeam"]], ignore_index=True)))

# Phễu lọc UI: Chỉ hiển thị các đội thuộc list WC_2026_TEAMS

filtered_teams = [t for t in teams if t in WC_2026_TEAMS]

if not filtered_teams:

    filtered_teams = teams # Đề phòng trường hợp database chưa có đội nào trong list thì hiển thị hết
...
with tabs[0]:
    ...
    with col1:
        ...
        show_cols = [c for c in ["Date","HomeTeam","AwayTeam","HomeScore","AwayScore","xG_Home","xG_Away","xGOT_Home","xGOT_Away","Shot_Quality_Home","Shooting_Efficiency_Home","Goalkeeping_Overperformance_Home","Rank_Diff"] if c in a_last.columns]
    ...
    with col2:
        ...
        show_cols = [c for c in ["Date","HomeTeam","AwayTeam","HomeScore","AwayScore","xG_Home","xG_Away","xGOT_Home","xGOT_Away","Shot_Quality_Home","Shooting_Efficiency_Home","Goalkeeping_Overperformance_Home","Rank_Diff"] if c in b_last.columns]

        st.dataframe(b_last[show_cols], use_container_width=True)

    st.subheader(f"H2H (latest {n}) - {team_a} vs {team_b}")

    show_cols = [c for c in ["Date","HomeTeam","AwayTeam","HomeScore","AwayScore","xG_Home","xG_Away","xGOT_Home","xGOT_Away","Shot_Quality_Home","Shooting_Efficiency_Home","Goalkeeping_Overperformance_Home","Rank_Diff"] if c in h2h_df.columns]
...
with tabs[1]:
    ...
    if "AHCh" not in df.columns:
        ...
        st.caption("AHCh lÃ  line cháº¥p gÃ³c cho Home (Ã¢m = Home cháº¥p). Win% tÃ­nh theo (W + 0.5*HW)/N.")
    ...
    if len(show_cols) > 3:
        ...
        st.caption("H2H không có snapshot corner handicap odds/line.")
...
with tabs[3]:
    ...
    if h2h_use.empty:
        ...
        show_cols = [c for c in ["Date","HomeTeam","AwayTeam","HomeScore","AwayScore","xG_Home","xG_Away","xGOT_Home","xGOT_Away","Shot_Quality_Home","Shooting_Efficiency_Home","Goalkeeping_Overperformance_Home","Rank_Diff"] if c in h2h_use.columns]
...
with tabs[4]:
    ...
    if "AHh" not in df.columns:
        ...
        if len(show_cols) > 3:
            ...
            st.caption("H2H không có snapshot odds/line.")

        st.markdown("### Gá»£i Ã½ Ä‘á»c nhanh")

        st.write("- **AHh** lÃ  line handicap cho Home (Ã¢m = Home cháº¥p).")

        if "AvgAHH" in df.columns and "AvgAHA" in df.columns:

            st.write("- **AvgAHH/AvgAHA** lÃ  odds trung bÃ¬nh cho Home/Away theo line Ä‘Ã³ (snapshot/closing-style).")
...
with tabs[5]:
    ...
    with left:
        ...
        team_a_stats_cache = extract_adv_metrics(a_last, team_a)
        team_b_stats_cache = extract_adv_metrics(b_last, team_b)

        summary_matched_odds = build_ai_odds_payload(st.session_state.get("latest_odds_api"), team_a, team_b)
        summary_matched_odds = filter_matched_odds_with_poisson(summary_matched_odds, team_a_stats_cache, team_b_stats_cache)
    ...
    if "AHCh" not in df.columns:
        ...
        st.info("File nÃ y chÆ°a cÃ³ cá»™t AHCh (Corner Asian Handicap).")
    ...
    with right:
        ...
        provider = st.selectbox("Provider", ["Gemini"], index=0, key="ai_provider")
        if provider == "Gemini":
            gemini_model_choice = "gemini-3.5-flash"
            api_url = f"https://generativelanguage.googleapis.com/{GEMINI_API_VERSION}/models/{normalize_gemini_model(gemini_model_choice)}:generateContent"
            gemini_key_choice = st.selectbox("Gemini key", ["Key 1", "Key 2", "Key 3", "Key 4", "Key 5"], index=0, key="gemini_key_choice")
            
            if gemini_key_choice == "Key 1": api_key = GEMINI_API_KEY
            elif gemini_key_choice == "Key 2": api_key = GEMINI_API_KEY_2
            elif gemini_key_choice == "Key 3": api_key = GEMINI_API_KEY_3
            elif gemini_key_choice == "Key 4": api_key = GEMINI_API_KEY_4
            else: api_key = GEMINI_API_KEY_5
            
            openai_model = None
            active_model_name = gemini_model_choice
            st.caption(f"Using Gemini model: {gemini_model_choice} ({GEMINI_API_VERSION}) | {gemini_key_choice}")
            if not api_key:
                st.warning("Missing selected Gemini key. Paste it at the top of worldcup_predict.py before running.")
        latest_odds = st.session_state.get("latest_odds_api")

        matched_odds = build_ai_odds_payload(latest_odds, team_a, team_b)
        matched_odds = filter_matched_odds_with_poisson(matched_odds, team_a_stats_cache, team_b_stats_cache)
        ...
        default_prompt = (
            ...
            f"- Corner Handicap: Home <line> OR Away <line>\n"

            f"- Cards: Over <line> OR Under <line> OR Home/Away cards +<x>\n"

            f"- Handicap: Home <line> OR Away <line>\n"
            ...
            f"{'⚠️ CẢNH BÁO TỪ HỆ THỐNG (CIRCUIT BREAKER): Chênh lệch đẳng cấp rất lớn giữa 2 đội. Hãy CÂN NHẮC KỸ: thống kê nâng cao của đội cửa dưới có thể bị thổi phồng do đá với đối thủ yếu. Đối chiếu xG raw vs xG SOS-adjusted để đánh giá.\n\n' if (matched_odds and matched_odds.get('poisson_picks', {}).get('is_blowout_risk')) else ''}"
            ...
            f"1. Ưu tiên tối thượng cho Dữ liệu nâng cao (Advanced Metrics):\n"

            f"   - Quyết định Handicap (Chấp bàn thắng) và BTTS dựa trên Net xG (Hiệu số xG), Shot Quality (Chất lượng cơ hội) và Chạm bóng trong vòng cấm (Touches in Box).\n"

            f"   - Quyết định Over/Under (Tài/Xỉu bàn thắng) dựa trên tổng xG, Shooting Efficiency (Độ bén hàng công) xung đột với Goalkeeping Overperformance (Phong độ thủ môn).\n"

            f"2. Hạ thấp trọng số của Hit Rate quá khứ: Tỷ lệ nổ kèo thô sơ trong quá khứ có độ nhiễu và biến động rất cao. Tuyệt đối không dùng Hit Rate để định đoạt kết quả nếu các chỉ số nâng cao (xG, xGOT) của phong độ gần đây phản mẫu hoặc trái ngược.\n"

            f"3. Tái cấu trúc Market Lean rule (Tie-Breaker): Tỷ lệ kèo nhà cái (giá thấp/cao) chỉ đóng vai trò làm bộ lọc xác nhận hoặc làm tín hiệu \"Tie-breaker\" khi chỉ số nâng cao của 2 đội cân bằng. Nếu Data nâng cao mâu thuẫn hoàn toàn với Market Lean, bạn được quyền đi NGƯỢC THỊ TRƯỜNG và ghi rõ nhãn 'contrarian vs market lean'.\n"

            f"4. Nới lỏng rào cản Odds (ODDS FILTER): Chỉ recommend một lựa chọn nếu giá cược của cửa đó (HK Price) >= 0.75. Tuy nhiên, nếu một cửa có dữ liệu nâng cao ủng hộ cực mạnh (xác suất thắng thực tế cao) nhưng giá nhà cái ra quá thấp (< 0.75), hãy chủ động QUÉT và CHỌN một thị trường khác (như Corners, 1H, Handicap phụ) thỏa mãn điều kiện >= 0.75 thay vì bỏ qua cơ hội (NO BET).\n"

            f"5. Nếu thông tin hoàn toàn mù mờ hoặc mâu thuẫn cực đoan giữa hệ thống chỉ số -> Output: NO BET.\n"

            f"6. QUAN TRỌNG: Data giao hữu (Pre-WC) có cỡ mẫu (samples) rất mỏng (từ 1-3 trận). TUYỆT ĐỐI KHÔNG output NO BET vì lý do thiếu data hoặc cỡ mẫu mỏng. Hãy ép bản thân đưa ra dự đoán tốt nhất dựa trên lượng data đang có.\n"

            f"{provider_extra_rules}"

            f"\n"

            f"HƯỚNG DẪN ÁNH XẠ CHỈ SỐ SANG KÈO THỰC TẾ:\n"

            f"- Goals (Over/Under): Bắt OVER nếu cả hai hàng công có Shooting Efficiency > 0 và hai thủ môn có Goalkeeping Overperformance <= 0. Bắt UNDER nếu hai thủ môn đang bay cao (Goalkeeping Overperformance > 0 lớn).\n"

            f"- Corners: Over/Handicap Góc chỉ chọn khi một đội có Blocked Shots (Cú sút bị chặn) tạo ra rất cao do lối chơi nhồi bóng dứt điểm liên tục khiến hậu vệ đối phương phải phá ra biên ngang.\n"

            f"- Cards: Chọn Over Thẻ nếu một đội có tỷ lệ tranh chấp tay đôi thành công tệ (Ground Duels < 45%) đối đầu với đội có tốc độ chuyển trạng thái nhanh thông qua số đường chuyền vào 1/3 cuối sân (Passes into Final Third) vượt trội.\n"

            f"- 1H (Hiệp 1): Chỉ đánh khi có dữ liệu tách biệt của Hiệp 1 cho thấy một đội có thói quen áp đặt trạng thái sớm (xG Hiệp 1 chiếm tỷ trọng lớn).\n"
            ...
            f"BACKUP: <Một lựa chọn kèo phụ hoặc '-'> | NOTE: <1 câu giải thích cực ngắn gọn chứa số liệu cụ thể của xG/xGOT/Biến phái sinh để chứng minh lý do chọn MAIN; Nếu đi ngược dòng tiền thị trường phải ghi rõ cụm từ 'contrarian vs market lean <side>'>\n"
        ...
        col_run, col_copy, col_math = st.columns([1, 1, 1])
        with col_run:
            run_clicked = st.button("Run AI", key="ai_run", use_container_width=True)
        with col_copy:
            copy_clicked = st.button("Copy Prompt (No API)", key="ai_copy", use_container_width=True)
        with col_math:
            math_clicked = st.button("Algorithmic Pick", key="ai_math", use_container_width=True)

        if math_clicked:
            if isinstance(matched_odds, dict) and matched_odds.get("poisson_picks"):
                picks = matched_odds["poisson_picks"]
                
                # --- NEW: Debug Panel ---
                with st.expander("🔍 Poisson Input Debug", expanded=False):
                    col_dbg1, col_dbg2 = st.columns(2)
                    with col_dbg1:
                        st.markdown(f"**{team_a} (HOME)**")
                        st.write(f"- xG (raw): {team_a_stats_cache.get('xG', 'N/A')}")
                        st.write(f"- xG (SOS-adj): {team_a_stats_cache.get('avg_xG', 'N/A')}")
                        st.write(f"- xGOT (raw): {team_a_stats_cache.get('xGOT', 'N/A')}")
                        st.write(f"- xGOT (SOS-adj): {team_a_stats_cache.get('avg_xGOT', 'N/A')}")
                        st.write(f"- Touches In Box: {team_a_stats_cache.get('avg_Touches_In_Box', 'N/A')}")
                        st.write(f"- GK Overperformance: {team_a_stats_cache.get('avg_Goalkeeping_Overperformance', 'N/A')}")
                        st.write(f"- Goals Prevented: {team_a_stats_cache.get('avg_Goals_Prevented', 'N/A')}")
                        st.write(f"- FIFA Rank: {team_a_stats_cache.get('latest_Rank', 'N/A')}")
                    with col_dbg2:
                        st.markdown(f"**{team_b} (AWAY)**")
                        st.write(f"- xG (raw): {team_b_stats_cache.get('xG', 'N/A')}")
                        st.write(f"- xG (SOS-adj): {team_b_stats_cache.get('avg_xG', 'N/A')}")
                        st.write(f"- xGOT (raw): {team_b_stats_cache.get('xGOT', 'N/A')}")
                        st.write(f"- xGOT (SOS-adj): {team_b_stats_cache.get('avg_xGOT', 'N/A')}")
                        st.write(f"- Touches In Box: {team_b_stats_cache.get('avg_Touches_In_Box', 'N/A')}")
                        st.write(f"- GK Overperformance: {team_b_stats_cache.get('avg_Goalkeeping_Overperformance', 'N/A')}")
                        st.write(f"- Goals Prevented: {team_b_stats_cache.get('avg_Goals_Prevented', 'N/A')}")
                        st.write(f"- FIFA Rank: {team_b_stats_cache.get('latest_Rank', 'N/A')}")
                    
                    st.markdown("---")
                    st.write(f"**Poisson Input:** home_xg={picks.get('_debug_home_xg', 'N/A')}, away_xg={picks.get('_debug_away_xg', 'N/A')}")
                    st.write(f"**Circuit Breaker:** {'🔴 TRIGGERED' if picks.get('is_blowout_risk') else '🟢 OFF'}")
                    if picks.get("debug"):
                        st.code("\n".join(str(d) for d in picks["debug"]), language="text")
                # --- END Debug Panel ---
                t_pick = picks.get('totals_pick')
                t_ev = picks.get('totals_ev')
                s_pick = picks.get('spreads_pick')
                s_ev = picks.get('spreads_ev')
                
                if picks.get("is_blowout_risk"):
                    st.error("🚨 **HIGH BLOWOUT RISK DETECTED (VỠ TRẬN)** 🚨\n\nHệ thống phát hiện thế trận áp đảo hoàn toàn. **ƯU TIÊN ĐÁNH TÀI VÀ NẰM CỬA TRÊN!**")

                def calculate_fixed_profit_stake(hk_odd, target_profit=300000, min_odd=0.85):
                    if hk_odd < min_odd:
                        return "SKIP (Odd < 0.85)"
                    stake = target_profit / hk_odd
                    return f"{int(round(stake, -3)):,} VNĐ"

                def extract_odd_and_calc_stake(pick_str):
                    if not pick_str or "NO BET" in pick_str or "(@ " not in pick_str and "(@" not in pick_str:
                        return ""
                    try:
                ...
                b_pick = picks.get('btts_pick')
                b_ev = picks.get('btts_ev')
                if t_pick or s_pick or b_pick:
                    t_stake = extract_odd_and_calc_stake(t_pick)
                    s_stake = extract_odd_and_calc_stake(s_pick)
                    b_stake = extract_odd_and_calc_stake(b_pick)

                    t_str = f"{t_pick} (EV: {t_ev*100:.2f}%) {f'- 💰 Stake: {t_stake}' if t_stake else ''}" if t_pick and t_pick != "NO BET" else t_pick if t_pick else "None"
                    s_str = f"{s_pick} (EV: {s_ev*100:.2f}%) {f'- 💰 Stake: {s_stake}' if s_stake else ''}" if s_pick and s_pick != "NO BET" else s_pick if s_pick else "None"
                    b_str = f"{b_pick} (EV: {b_ev*100:.2f}%) {f'- 💰 Stake: {b_stake}' if b_stake else ''}" if b_pick and b_pick != "NO BET" else b_pick if b_pick else "None"
                    combo_str = picks.get("recommended_combo", "")
                    
                    st.success(f"**🤖 Poisson Algorithmic Picks & Financial Management:**\n\n"
                               f"⚽ **Totals:** {t_str}\n\n"
                               f"📐 **Handicap:** {s_str}\n\n"
                               f"🤝 **BTTS:** {b_str}\n\n"
                               f"---\n\n"
                    ...
                    st.warning("No Algorithmic Pick available for this match.")
                    st.write("Debug info - Parsed odds lines:")
                    st.write(picks.get("debug", []))
                    st.write("RAW BOOKMAKERS PAYLOAD:")
                    try:
                        import json
                        raw_data = st.session_state.get("latest_odds_api", {}).get("event", {}).get("bookmakers", [])
                        st.code(json.dumps(raw_data, indent=2), language="json")
                    except:
                        pass
            else:
                st.warning("No odds payload found for this match.")

        if run_clicked or copy_clicked:

            market_snapshot = {}

            if not h2h_df.empty and "Date" in h2h_df.columns:

                latest = h2h_df.sort_values("Timestamp", ascending=False).iloc[0]
            ...
            payload = {
                ...
                "advanced_metrics": {
                    ...
                    team_a: extract_adv_metrics(a_last, team_a),

                    team_b: extract_adv_metrics(b_last, team_b)
            ...
            if copy_clicked:
                ...
                st.warning("VÃ¬ báº¡n Ä‘ang cháº¡y app qua máº¡ng ná»™i bá»™ (IP) nÃªn nÃºt Copy gÃ³c pháº£i cÃ³ thá»ƒ bá»‹ trÃ¬nh duyá»‡t cháº·n. HÃ£y click chuá»™t vÃ o khung dÆ°á»›i, báº¥m **Ctrl + A** Ä‘á»ƒ bÃ´i Ä‘en táº¥t cáº£, rá»“i **Ctrl + C** Ä‘á»ƒ copy nhÃ©:")

                st.text_area("Ná»™i dung Payload (JSON)", value=json.dumps(payload, ensure_ascii=False, separators=(',', ':'), default=str), height=150, key="payload_text_area")
        ...
        if not res.get("ok"):
            ...
            if not raw:
                ...
                for item in raw:
                    ...
                    if "bos" in home_name.lower() or "bos" in away_name.lower() or "can" in home_name.lower() or "can" in away_name.lower():
                        print(f"DEBUG_API_TEAM: {home_name} vs {away_name}")
                        
                    match_forward = _team_names_match(home_name, team_a_norm) and _team_names_match(away_name, team_b_norm)
                    match_reverse = _team_names_match(home_name, team_b_norm) and _team_names_match(away_name, team_a_norm)
